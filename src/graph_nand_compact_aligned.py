...
def generate_circuit_graph(circuit: Circuit, filename: str = None, format: str = "png"):
    ...
    _build_circuit_graph(
        main_graph, circuit, "", wire_connections, is_main_circuit=True
    )
...
def _build_circuit_graph(
    parent_graph, circuit, prefix, wire_connections, is_main_circuit=False
):
    """
    ...
    if circuit.identifier == 0:
        # Create ports for NAND gate I/O
        ...
        # Add output port
        out_key = next(iter(circuit.outputs))
        out_node_id = f"{prefix}_out_{out_key}"
        port_nodes[("out", out_key)] = out_node_id
        parent_graph.add_node(
            pydot.Node(
                out_node_id,
                label=f"{out_key}",
        ...
        for in_key, wire in circuit.inputs.items():
            parent_graph.add_edge(pydot.Edge(port_nodes[("in", in_key)], nand_node_id))

        # Connect the NAND node to the output
        parent_graph.add_edge(pydot.Edge(nand_node_id, port_nodes[("out", out_key)]))

        return port_nodes

    # For non-NAND circuits, create a subgraph unless it's the main circuit
    ...
    # Add output nodes (for non-NAND circuits or later in NAND special case)
    output_ids = []
    ...
    # Add rank constraints for inputs and outputs:
    if input_ids:
        subgraph_input = pydot.Subgraph("subgraph_input_" + prefix)
        subgraph_input.set_rank("min")
        # Listing node names forces these nodes to be ranked at the minimum (leftmost)
    ...
    if output_ids:
        ...
        # Listing node names forces these nodes to be ranked at the maximum (rightmost)
        for nid in output_ids:
            subgraph_output.add_node(pydot.Node(nid))
        graph_to_use.add_subgraph(subgraph_output)
    ...
    for comp_key, component in circuit.components.items():
        ...
        if component.identifier == 0:
            ...
            component_port_nodes = {
                ("in", in_key): nand_node_id for in_key in component.inputs.keys()
            }
            component_port_nodes.update(
                {("out", out_key): nand_node_id for out_key in component.outputs.keys()}
            )
    ...
    for circuit_out_key, wire in circuit.outputs.items():
        ...
        for comp_key, component in circuit.components.items():
            for comp_out_key, comp_wire in component.outputs.items():
                if comp_wire.id == wire_id:
                    # For NAND gates, connect directly from the NAND node
                    if component.identifier == 0:
            ...
            for tgt_comp_key, tgt_component in circuit.components.items():
                ...
                for tgt_in_key, tgt_wire in tgt_component.inputs.items():
                    if tgt_wire.id == src_wire_id:
                        # For NAND gates, use the direct node references
                        if src_component.identifier == 0:
                        ...
                        connection = (src_wire_id, source_node, target_node)
                        if connection not in wire_connections:
...
if __name__ == "__main__":
    ...
    visualize_schematic(8, reference, "2bitsadder_chatgpt")
