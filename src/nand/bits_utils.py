...
def read_bits(data: List[int], n: int) -> int:
    ...
    if len(data) < n:
        raise ValueError(
            f"Not enough bits in data (of len {len(data)}) to form an integer with {n} requested bits."
        )
...
def read_bits_with_offset(data: List[int], n: int) -> int:
    """Convert the first n bits from a list (integer of value 0 or 1) to an integer, with an offset of +1

    This offset make a '0' become 1, '1' become 2, '10' become 3, etc. See 'BitPackedEncoder' for rational.
    """
...
def bitlength_with_offset(n: int):
    ...
    This offset makes for example the value 8, which would be encoded as '1000' so with a bitlength of 4,
    into the value 7, which is encoded as '111' so with a bitlength and so return value of 3.
...
def int2bitlist_with_offset(n: int, bit_size: int):
    ...
    This offset makes for example the value 8, which would be returned as [1, 0, 0, 0], into
    the value 7, which is encoded as [1, 1, 1] which is the effective return value.
