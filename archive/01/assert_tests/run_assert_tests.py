...
def main():
    ...
    if len(failed_executables) != 0:
        print(
            "KO! The following tests either should have failed, or aborted for another reason:"
        )
        for executable in failed_executables:
            print(f"\t{executable}")
    else:
        print(
            f"ok. All assertions were correctly triggered: {len(executables)} tests executed"
        )
