...
def _build_circuit_graph(
    parent_graph, circuit: Circuit, prefix, wire_connections, is_main_circuit=False
):
    """
    ...
    if circuit.identifier == 0:
        ...
        for out_key, wire in circuit.outputs.items():
            parent_graph.add_edge(
                pydot.Edge(nand_node_id, port_nodes[("out", out_key)])
            )
    else:
                    ...
                    for tgt_in_key, tgt_wire in tgt_component.inputs.items():
                        if tgt_wire.id == src_wire_id:
                            ...
                            if connection not in wire_connections:
                                parent_graph.add_edge(
                                    pydot.Edge(
                                        component_ports[src_comp_key][
                                            ("out", src_out_key)
                                        ],
                                        component_ports[tgt_comp_key][
                                            ("in", tgt_in_key)
                                        ],
                                    )
                                )
