...
from tests.sequence_utils import _flatten
from tests.common_assertions import assert_circuit_signature, assert_simulation_result
...
def assert_basic_gate(
    ...
    for case in input_cases:
        ...
        assert_simulation_result(result, case, expected, simulator._circuit.name)
...
def assert_dmux_n_way(simulator: Simulator, n_way: int):
    ...
    for in_ in [True, False]:
        ...
        for sel_int in range(n_way):
            ...
            assert_simulation_result(
                result, input_list, expected, simulator._circuit.name
            )
