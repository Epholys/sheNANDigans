...
class EncodedCircuitMetadata:
    def __init__(self):
        ...
        self.component_rolling_bitlength: int = 0
        self.component_rolling_count: int = 0
        self.component_idx_map: dict[CircuitId, int] = {}
...
class RollingEncoder(CircuitEncoder):
    """
    ...
    def encode(self, library: CircuitLibrary) -> bitarray:
        ...
        print(
            f"core bl = {core_bitlength} ; circuit bl = {self.circuits_bitlength} ; comp bl = {max_components_bitlength} ; in bl = {max_inputs_bitlength} ; out bl = {max_outputs_bitlength}"
        )
        ...
        for data, bitlength in self.int_encoding:
            match bitlength:
                case Placeholder.COMPONENTS:
                    print(
                        f"n component = {data} encoded in {max_components_bitlength} bits"
                    )
                    bit_encoding.extend(
    ...
    def _encode_circuit(self, circuit: Circuit):
        ...
        """
        print("")
        print(f"encode circuit : {circuit.name}")
    ...
    def _encode_component(
        ...
        print(
            f"encode component : {component.name} (index {circuit_ids.index(component.identifier)}) in {self.circuits_bitlength} bits"
        )
    ...
    def _encode_inputs(
        ...
        for input_wire in component.inputs.values():
            if input_wire.id in circuit_input:
                ...
                print(
                    f"encode input from circuit at {circuit_input.index(input_wire.id)} in {metadata.inputs_bitlength} bits"
                )
                self.int_encoding.append(
    ...
    def _encode_component_wiring(
        ...
        for idx, sub_component in enumerate(components.values()):
            ...
            if wire.id in outputs:
                ...
                self._update_bl_by_one(metadata)
                return
        raise ValueError(f"Wire {wire.id} not found in any sub_component outputs")

    def _update_rolling_bl(self, component_idx: int, metadata: EncodedCircuitMetadata):
        print("- update -")
        if component_idx not in metadata.component_idx_map:
            print(
                f"key {component_idx} not in map ; SET component idx map [key] as {metadata.component_rolling_count}"
            )
            metadata.component_idx_map[component_idx] = metadata.component_rolling_count
            print("increment rolling count")
            metadata.component_rolling_count += 1
            print(
                f"compute component count bl (w/ offset) : for {metadata.component_rolling_count} bl is {bitlength_with_offset(metadata.component_rolling_count)}"
            )
